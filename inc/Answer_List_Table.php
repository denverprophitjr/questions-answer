...
class DWQA_Answer_List_Table extends WP_List_Table {
	...
	function display() {
		...
		$this->prepare_items();
		?>
        <table class="wp-list-table dwqa-answer-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>"
               cellspacing="0">
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'detail':
				...
				setup_postdata( $item );
				?>
                <div class="submitted-on"><?php _e( 'Answered on ', 'dwqa' ) ?><a
                            href="<?php echo get_permalink( $item->ID ) ?>"><?php echo $item->post_date ?></a></div>
				<?php the_excerpt(); ?>
				<?php
				break;
			default:
