...
function dwqa_question_status_box_html( $post ) {
    ...
    <p>
        <label for="dwqa-question-status">
            ...
            <select name="dwqa-question-status" id="dwqa-question-status" class="widefat">
                ...
                <option <?php selected( $meta, 'resolved' ); ?>
    ...
    <p>
        <label for="dwqa-question-sticky">
			...
			?>
            <input <?php checked( true, in_array( $post->ID, $sticky_questions ), true ); ?> type="checkbox"
                                                                                             name="dwqa-question-sticky"
                                                                                             id="dwqa-question-sticky"
                                                                                             value="1"><span
                    class="description"><?php _e( 'Pin question to top of archive page.', 'dwqa' ); ?></span>
        </label>
    </p>
	<?php
}
...
class DWQA_Metaboxes {
	...
	public function answers_metabox() {
		add_meta_box( 'dwqa-answers', __( 'Answers', 'dwqa' ), array(
			$this,
			'metabox_answers_list'
		), 'dwqa-question' );
	}
