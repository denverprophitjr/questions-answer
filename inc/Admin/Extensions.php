...
class DWQA_Admin_Extensions {
	...
	function register_extension_menu() {
		add_submenu_page( 'edit.php?post_type=dwqa-question', __( 'Extensions', 'dwqa' ), sprintf( '<span style="color: #d54e21;">%s</span>', __( 'Extensions', 'dwqa' ) ), 'manage_options', 'dwqa-extensions', array(
			$this,
			'extension_menu_layout'
		) );
	}
                        ...
                        <div class="theme">
							<?php if ( ! empty( $info['img_url'] ) ) : ?>
                                <div class="theme-screenshot">
                                    <a target="_blank" href="<?php echo esc_url( $info['url'] ) ?>"><img
                                                src="<?php echo esc_url( $info['img_url'] ) ?>"></a>
                                </div>
							<?php else : ?>
                            ...
                            <h2 class="theme-name" id="<?php echo esc_attr( $slug ) ?>">
                                <span><?php echo esc_attr( $info['name'] ) ?></span></h2>
                            <div class="theme-actions">
                                <a class="button button-primary" target="_blank"
                                   href="<?php echo esc_url( $info['url'] ) ?>"><?php _e( 'Get It Now!', 'dwqa' ); ?></a>
                            </div>
                        </div>
					<?php endforeach; ?>
                </div>
