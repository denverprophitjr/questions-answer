...
<div class="<?php echo dwqa_post_class() ?>" id="answer-<?php the_ID() ?>"
    ...
    <aside class="dwqa-answer-meta">
		...
		<?php printf( __( '<span><a href="%1$s"><span itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name">%2$s%3$s</span></span></a> %4$s answered %5$s ago</span>', 'dwqa' ), dwqa_get_author_link( $user_id ), get_avatar( $user_id, 48 ), get_the_author(), dwqa_print_user_badge( $user_id ), human_time_diff( get_post_time( 'U', false ), current_time( 'timestamp' ) ) ) ?>
