    ...
    <div class="dwqa-comment-meta">
        ...
        <div class="dwqa-comment-actions">
			<?php if ( dwqa_current_user_can( 'edit_comment', $comment->comment_ID ) || dwqa_current_user_can( 'manage_comment' ) ) : ?>
                <a class="dwqa-edit-comment"
                   href="<?php echo esc_url( add_query_arg( array( 'comment_edit' => $comment->comment_ID ) ) ) ?>"><?php _e( 'Edit', 'dwqa' ) ?></a>
			<?php endif; ?>
			<?php if ( dwqa_current_user_can( 'delete_comment', $comment->comment_ID ) || dwqa_current_user_can( 'manage_comment' ) ) : ?>
                <a class="dwqa-delete-comment" href="<?php echo wp_nonce_url( add_query_arg( array(
					'action' => 'dwqa-action-delete-comment',
					'comment_id' => $comment->comment_ID
				), admin_url( 'admin-ajax.php' ) ), '_dwqa_delete_comment' ) ?>"><?php _e( 'Delete', 'dwqa' ) ?></a>
			<?php endif; ?>
        </div>
    </div>
	<?php comment_text(); ?>
