...
<?php if ( dwqa_current_user_can( 'edit_' . $type, $edit_id ) || dwqa_current_user_can( 'manage_' . $type ) ) : ?>
    ...
    <form method="post" class="dwqa-content-edit-form" enctype="multipart/form-data">
		...
		<?php if ( 'dwqa-question' == get_post_type( $edit_id ) ) : ?>
            ...
            <p>
                ...
                <input type="text" class="" name="question-tag"
                       value="<?php dwqa_get_tag_list( get_the_ID(), true ); ?>">
            </p>
		<?php endif; ?>
        ...
        <input type="submit" name="dwqa-edit-<?php echo $type ?>-submit" class="dwqa-btn dwqa-btn-primary"
               value="<?php _e( 'Save Changes', 'dwqa' ) ?>">
    </form>
